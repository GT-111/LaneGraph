...
for region in regions:
	...
	for ilat in range(blocks[0]):
		for ilon in range(blocks[1]):
			...
			img = cv2.imread(raw_data_path + "/sat_%d.jpg" % (counter))
			try:
				labels = pickle.load(open(raw_data_path + "/sat_%d_label.p" % (counter), "rb"))
			except:
				break
			...
			for sr in [0, 1024, 2048]:
				for sc in [0, 1024, 2048]:
					...
					# cv2.imwrite(processed_data_path + "/sat_%d.jpg" % (counter_out), sat)
					# cv2.imwrite(processed_data_path + "/regionmask_%d.jpg" % (counter_out), mask)
					# cv2.imwrite(processed_data_path + "/lane_%d.jpg" % (counter_out), lane)
					# cv2.imwrite(processed_data_path + "/normal_%d.jpg" % (counter_out), normal)
