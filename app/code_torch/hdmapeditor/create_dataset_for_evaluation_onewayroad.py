    ...
    for ilat in range(blocks[0]):
        for ilon in range(blocks[1]):
            ...
            for sr in [0]:
                for sc in [0]:
                    localways = json.load(
                        open(outputfolder + "/way_%d.json" % (counter_out), "r")
                    )
                    ...
                    for gid_ in range(gid):
                        ...
                        for i in range(len(localways)):
                            if group_id[i] == gid_:
                                directions.append(
                                    (
                                        localways[i][-1][0] - localways[i][0][0],
                                        localways[i][-1][1] - localways[i][0][1],
                                    )
                                )
                    ...
                    json.dump(
                        group_id_list,
                        open(outputfolder + "/group_%d.json" % (counter_out), "w"),
                        indent=2,
                    )
