        ...
        if editingMode == "ready_to_draw":
            ...
            if existing_node is not None:
                ...
                if autofill_mode == "ml":
                    ...
                    sat = image_for_cnn[
                        mr - cnnsize // 2 : mr + cnnsize // 2,
                        mc - cnnsize // 2 : mc + cnnsize // 2,
                        :,
                    ]
                    ...
                    cv2.circle(
                        connector,
                        (c1 - (mc - cnnsize // 2), r1 - (mr - cnnsize // 2)),
                        8,
                        (255, 255, 255),
                        -1,
                    )
                    cv2.circle(
                        connector,
                        (c2 - (mc - cnnsize // 2), r2 - (mr - cnnsize // 2)),
                        8,
                        (255, 255, 255),
                        -1,
                    )
            ...
            for nid, loc in laneMap.nodes.items():
                if (
                    ...
                    if (
                        loc[1] > global_y - erase_size * 50 // zoom
...
def redraw(noshow=False, transpose=False):
    ...
    frame = np.copy(
        image[
            pos[1] : pos[1] + windowsize[1] // zoom,
            pos[0] : pos[0] + windowsize[0] // zoom,
            :,
        ]
    )
    if zoom > 1:
                        ...
                        if renderpass == 0:
                            for mx, my in ms:
                                if not vis_switch_no_arrow:
                                    cv2.line(
                                        frame,
                                        (mx + int(dx * scale), my + int(dy * scale)),
        ...
        for nid, p in laneMap.nodes.items():
            ...
            if laneMap.nodeType[nid] == "way":
                for nei in laneMap.neighbors_all[nid]:
                    if (
                        (nei, nid) in laneMap.edgeType
                        and laneMap.edgeType[(nei, nid)] == "link"
                    ) or (
                        (nid, nei) in laneMap.edgeType
                        and laneMap.edgeType[(nid, nei)] == "link"
                    ):
    ...
    # draw virtual lines
    if (
        editingMode == "drawing_polyline" or editingMode == "autofill_stage2"
    ) and lastNodeID is not None:
    ...
    crop = cv2.resize(
        frame,
        (
            int(float(windowsize[0] // zoom) / imageDim[0] * 256),
            int(float(windowsize[1] // zoom) / imageDim[1] * 256),
        ),
    ...
    if vis_switch_no_minimap == False:
        ...
        x2 = (
            ...
            - 256
            - 10
            + int(float(pos[0] + windowsize[0] // zoom) / imageDim[0] * 256)
        )
    ...
    if editingMode == "erase":
        ...
        cv2.rectangle(
            frame,
            (mousex - erase_size * 50, mousey - erase_size * 50),
            (mousex + erase_size * 50, mousey + erase_size * 50),
            color,
            2,
        )
    ...
    if noshow == False:
        cv2.putText(
            frame,
            "%s | %s | Layer %d | Render Time %.3f Seconds"
...
if len(sys.argv) > 3:
    ...
    vis_switch_no_direction = (
        config["no_direction"] if "no_direction" in config else vis_switch_no_direction
    )
    vis_switch_no_arrow = (
        config["no_arrow"] if "no_arrow" in config else vis_switch_no_arrow
    )
    vis_switch_no_minimap = (
        config["no_minimap"] if "no_minimap" in config else vis_switch_no_minimap
    )
    vis_switch_no_diff_way_and_link = (
        config["no_diff_way_and_link"]
        if "no_diff_way_and_link" in config
        else vis_switch_no_diff_way_and_link
    )
    vis_switch_no_vertices = (
        config["no_vertices"] if "no_vertices" in config else vis_switch_no_vertices
    )
    vis_switch_no_way = config["no_way"] if "no_way" in config else vis_switch_no_way
    vis_switch_no_link = (
        config["no_link"] if "no_link" in config else vis_switch_no_link
    )
