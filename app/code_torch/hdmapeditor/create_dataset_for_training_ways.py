...
for region in regions:
    ...
    for ilat in range(blocks[0]):
        for ilon in range(blocks[1]):
            ...
            for way in ways:
                node_list = [
                    (roadlabel.nodes[nid][0], roadlabel.nodes[nid][1]) for nid in way
                ]
                density = 50
                pd = [0]
                ...
                for i in range(interpolate_N):
                    ...
                    for j in range(len(node_list) - 1):
                        if pd[j] <= int_d and pd[j + 1] > int_d:
                            ...
                            loc = (
                                (1 - a) * node_list[j][0] + a * node_list[j + 1][0],
            ...
            for sr in [0, 1024, 2048]:
                for sc in [0, 1024, 2048]:
                    mask = cv2.imread(
                        outputfolder + "/regionmask_%d.jpg" % (counter_out)
                    )
                    margin = 128
                    ...
                    for way in ways:
                        ...
                        for loc in way:
                            ...
                            if last_x is not None:
                                if (x > 0 and x < 2048 and y > 0 and y < 2048) or (
                                    last_x > 0
                                    and last_x < 2048
                                    and last_y > 0
                                    and last_y < 2048
                    ...
                    json.dump(
                        localways,
                        open(outputfolder + "/way_%d.json" % (counter_out), "w"),
                        indent=2,
                    )
                    pickle.dump(
                        neighbors,
                        open(outputfolder + "/graph_%d.p" % (counter_out), "wb"),
                        protocol=2,
                    )
                    ...
                    cv2.imwrite(
                        outputfolder + "/graphvis_%d.jpg" % (counter_out), graph_vis
                    )
