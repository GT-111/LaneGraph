...
def resnet34unet(x, is_training, ch_in=3, ch_out=2, ch=64):
    x, _, _ = common.create_conv_layer(
        "enc_1",
        x,
        ch_in,
        ch,
    ...
    def aggregate_block(
        x1, x2, in_ch1, in_ch2, out_ch, name, batchnorm=True, activation="relu"
    ):
        x2, _, _ = common.create_conv_layer(
            name + "_1",
            x2,
            in_ch2,
            in_ch2,
    ...
    x = aggregate_block(
        features[1], x, enc_dim[1], enc_dim[2], enc_dim[1], "agg2", batchnorm=True
    )
    x = aggregate_block(
        features[0], x, enc_dim[0], enc_dim[1], enc_dim[0], "agg3", batchnorm=True
    )
...
def resnet34unet_v3(
    ...
    features[3], _, _ = common.create_conv_layer(
        "mid_1",
        features[3],
        enc_dim[3],
        enc_dim[3],
    ...
    if feature_out:
        f_out, _, _ = common.create_conv_layer(
            "f_1",
            features[3],
            enc_dim[3],
            ch * 4,
    ...
    x = aggregate_block(
        ...
        "agg1",
        batchnorm=True,
    )  # 2: (None, 40, 40, 256), 3: (None, 20, 20, 512), x: (None, 40, 40, 256)
    # p16 = prediction_conv(x, enc_dim[2], ch_out, "pred16")
    x = aggregate_block(
        features[1], x, enc_dim[1], enc_dim[2], enc_dim[1], "agg2", batchnorm=True
    )  # 1: (None, 80, 80, 128), x: (None, 40, 40, 256), x: (None, 80, 80, 128)
    # p8 = prediction_conv(x, enc_dim[1], ch_out, "pred8")
    x = aggregate_block(
        features[0], x, enc_dim[0], enc_dim[1], enc_dim[0], "agg3", batchnorm=True
    )  # 1: (None, 160, 160, 64), x: (None, 80, 80, 128), x: (None, 160, 160, 64)
    # p4 = prediction_conv(x, enc_dim[0], ch_out, "pred4")
    x = aggregate_block(
        x1, x, ch * 2, enc_dim[0], ch, "agg4", batchnorm=True
    )  # x1: (None, 320, 320, 128), x: (None, 160, 160, 64), x: (None, 320, 320, 64)
    # p2 = prediction_conv(x, ch, ch_out, "pred2")
    p1, _, _ = common.create_conv_layer(
        "output",
        x,
        ch,
        ch_out,
        ...
        deconv=True,
        activation="linear",
    )  # x: (None, 320, 320, 64)->p1L: (None, 640, 640, 4)
...
def resnet34FPN(x, is_training, input_dim, ch_in=3, ch_out=2, ch=64, FPN_channel=256):
    ...
    x1 = x
    features = resnet_template(
        x, is_training=is_training, res_n=34, ch=ch, feature_activation=lambda x: x
    )
    features = [features[i] for i in range(len(features))]
    enc_dim = [ch, ch * 2, ch * 4, ch * 8]
    # 4x 8x 16x 32x
    ...
    def aggregate_block(
        x1, x2, in_ch1, in_ch2, out_ch, name, batchnorm=True, activation="relu", scale=1
    ):
        x2 = tf.image.resize(
            x2, [input_dim // scale, input_dim // scale], method="nearest"
        )
        x1, _, _ = common.create_conv_layer(
            name + "_1",
            x1,
            in_ch1,
            in_ch2,
    ...
    x32, _, _ = common.create_conv_layer(
        "turningpoint_conv",
        features[3],
        enc_dim[3],
        FPN_channel,
...
def resnet_template(
    x, is_training=True, reuse=False, res_n=18, ch=64, feature_activation=tf.nn.relu
):
    with tf.variable_scope("resnet", reuse=reuse):
        ...
        f1 = feature_activation(
            batch_norm_resnet(x, is_training, scope="batch_norm_f1")
        )
        ########################################################################################################
        ...
        f2 = feature_activation(
            batch_norm_resnet(x, is_training, scope="batch_norm_f2")
        )
        ########################################################################################################
        ...
        f3 = feature_activation(
            batch_norm_resnet(x, is_training, scope="batch_norm_f3")
        )
        ########################################################################################################
        ...
        f4 = feature_activation(
            batch_norm_resnet(x, is_training, scope="batch_norm_f4")
        )
        ########################################################################################################
