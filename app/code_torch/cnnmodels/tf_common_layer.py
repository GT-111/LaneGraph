...
def uniform(shape, scale=0.05, name=None):
    ...
    return tf.get_variable(
        name,
        shape=shape,
        initializer=tf.initializer.random_uniform(minval=-scale, maxval=scale),
        dtype=tf.float32,
    )
...
def glorot(shape, name=None):
    ...
    return tf.get_variable(
        name, shape=shape, initializer=tf.glorot_uniform_initializer(), dtype=tf.float32
    )
...
def zeros(shape, name=None):
    ...
    return tf.get_variable(
        name, shape=shape, initializer=tf.constant_initializer(0.0), dtype=tf.float32
    )
...
def ones(shape, name=None):
    ...
    return tf.get_variable(
        name, shape=shape, initializer=tf.constant_initializer(1.0), dtype=tf.float32
    )
...
def create_conv_layer(
    ...
    if deconv == False and nopadding == False:
        input_tensor = tf.pad(
            ...
            [
                [0, 0],
                [(kx + (dilation - 1) * 2) // 2, (kx + (dilation - 1) * 2) // 2],
                [(kx + (dilation - 1) * 2) // 2, (kx + (dilation - 1) * 2) // 2],
                [0, 0],
            ],
            mode="CONSTANT",
    ...
    weights = tf.get_variable(
        name + "weights",
        shape=[kx, ky, in_channels, out_channels],
        initializer=tf.truncated_normal_initializer(
            stddev=np.sqrt(0.02 / kx / ky / in_channels)
        ),
    ...
    if deconv == False:
        ...
        t = tf.nn.conv2d_transpose(
            input_tensor,
            tf.transpose(weights, perm=[0, 1, 3, 2]),
            [batch, size * stride_x, size * stride_y, out_channels],
            [1, stride_x, stride_y, 1],
            padding="SAME",
            data_format="NHWC",
        )
