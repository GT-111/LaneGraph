...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    for model_ep in [490]:
        if backbone == "resnet34v3":
            model.restoreModel(
                "model_4cities_run2_640_%s_500ep/model%d" % (backbone, model_ep)
            )
        else:
            model.restoreModel(
                "model_4cities_run1_640_%s_500ep/model%d" % (backbone, model_ep)
            )
        ...
        for i in range(dim[0] // windowsize1):
            ...
            for j in range(dim[1] // windowsize1):
                ...
                output[r : r + cnninput, c : c + cnninput, :] += (
                    x_out[0, :, :, :] * mask
                )
    ...
    Image.fromarray(((output[:, :, 0]) * 255).astype(np.uint8)).save(
        outputfolder + "/seg.png"
    )
