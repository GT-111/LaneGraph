        ...
        for i in range(self.preload_tiles if ind is None else 1):
            while True:
                ...
                if self.testing == False and random.randint(0, 5) < 4:
                    ...
                    normal = scipy.ndimage.rotate(
                        normal, angle, reshape=False, cval=127
                    )
                ...
                for k in links[1].keys():
                    ...
                    for j in range(len(links[2])):
                        if (
                            links[2][j][0][0] == pos[0] and links[2][j][0][1] == pos[1]
                        ) or (
                            links[2][j][-1][0] == pos[0]
                            and links[2][j][-1][1] == pos[1]
                        ):
                            linkids.append(j)
        ...
        for i in range(batchsize):
            while True:
                ...
                if coin == 0:
                    ...
                    if (
                        x1 < 0
                        or x1 >= self.image_size
                        or x2 < 0
                        or x2 >= self.image_size
                    ):
                        continue
                    if (
                        y1 < 0
                        or y1 >= self.image_size
                        or y2 < 0
                        or y2 >= self.image_size
                    ):
                        continue
                    ...
                    self.image_batch[i, :, :, :] = self.images[
                        tile_id,
                        sr + bx : sr + bx + self.image_size,
                        sc + by : sc + by + self.image_size,
                        :,
                    ]
                    ...
                    self.normal_batch[i, :, :, :] = self.normal[
                        tile_id, sr : sr + self.image_size, sc : sc + self.image_size, :
                    ]
