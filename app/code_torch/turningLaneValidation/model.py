...
class LinkModel:
    def __init__(self, sess, size=640, batchsize=4):
        ...
        self.position_code = tf.placeholder(
        ...
        input_data = tf.concat(
            [self.input, self.connector, self.context, self.position_code], axis=3
        )

        with tf.variable_scope("seg"):
            input_data1 = tf.concat(
                [
                    self.input,
                    self.connector[:, :, :, 0:3],
                    self.context,
            ...
            output_seg1 = resnet34unet_v3(
                input_data1, self.is_training, ch_in=10, ch_out=2, feature_out=False
            )

        with tf.variable_scope("seg", reuse=tf.AUTO_REUSE):
            input_data2 = tf.concat(
                [
                    self.input,
                    self.connector[:, :, :, 3:6],
                    self.context,
            ...
            output_seg2 = resnet34unet_v3(
                input_data2, self.is_training, ch_in=10, ch_out=2, feature_out=False
            )

        self.output = tf.concat(
            [
                tf.nn.softmax(output_seg1)[:, :, :, 0:1],
                tf.nn.softmax(output_seg2)[:, :, :, 0:1],
            ],
            axis=3,
        )
        ...
        with tf.variable_scope("class"):
            ...
            output_label = resnet18classifier(
                input_data, self.is_training, ch_in=13, ch_out=2
            )

        self.output_label = tf.nn.softmax(output_label)

        self.seg_loss = self.singlescaleloss(
            output_seg1[:, :, :, 0:2], self.target[:, :, :, 1:2], 1.0
        )
        self.seg_loss += self.singlescaleloss(
            output_seg2[:, :, :, 0:2], self.target[:, :, :, 2:3], 1.0
        )
        self.class_loss = tf.reduce_mean(
            tf.nn.softmax_cross_entropy_with_logits(
                labels=tf.concat([self.target_label, 1 - self.target_label], axis=1),
                logits=output_label,
            )
        )
